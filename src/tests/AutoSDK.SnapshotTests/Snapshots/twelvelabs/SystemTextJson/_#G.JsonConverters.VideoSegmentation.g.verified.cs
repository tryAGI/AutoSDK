//HintName: G.JsonConverters.VideoSegmentation.g.cs
#nullable enable
#pragma warning disable CS0618 // Type or member is obsolete

namespace G.JsonConverters
{
    /// <inheritdoc />
    public class VideoSegmentationJsonConverter : global::System.Text.Json.Serialization.JsonConverter<global::G.VideoSegmentation>
    {
        /// <inheritdoc />
        public override global::G.VideoSegmentation Read(
            ref global::System.Text.Json.Utf8JsonReader reader,
            global::System.Type typeToConvert,
            global::System.Text.Json.JsonSerializerOptions options)
        {
            options = options ?? throw new global::System.ArgumentNullException(nameof(options));
            var typeInfoResolver = options.TypeInfoResolver ?? throw new global::System.InvalidOperationException("TypeInfoResolver is not set.");

            using var __jsonDocument = global::System.Text.Json.JsonDocument.ParseValue(ref reader);
            var __rawJson = __jsonDocument.RootElement.GetRawText();
            var __jsonProps = new global::System.Collections.Generic.HashSet<string>();
            if (__jsonDocument.RootElement.ValueKind == global::System.Text.Json.JsonValueKind.Object)
            {
                foreach (var __jsonProp in __jsonDocument.RootElement.EnumerateObject())
                {
                    __jsonProps.Add(__jsonProp.Name);
                    if (__jsonProp.Value.ValueKind == global::System.Text.Json.JsonValueKind.Object)
                    {
                        foreach (var __nestedJsonProp in __jsonProp.Value.EnumerateObject())
                        {
                            __jsonProps.Add(__jsonProp.Name + "." + __nestedJsonProp.Name);
                        }
                    }

                }
            }

            var __score0 = 0;
            if (__jsonProps.Contains("dynamic")) __score0++;
            if (__jsonProps.Contains("dynamic.min_duration_sec")) __score0++;
            if (__jsonProps.Contains("strategy")) __score0++;
            var __score1 = 0;
            if (__jsonProps.Contains("fixed")) __score1++;
            if (__jsonProps.Contains("fixed.duration_sec")) __score1++;
            if (__jsonProps.Contains("strategy")) __score1++;
            var __bestScore = 0;
            var __bestIndex = -1;
            if (__score0 > __bestScore) { __bestScore = __score0; __bestIndex = 0; }
            if (__score1 > __bestScore) { __bestScore = __score1; __bestIndex = 1; }

            global::G.VideoSegmentation0? videoSegmentation0 = default;
            global::G.VideoSegmentation1? videoSegmentation1 = default;
            if (__bestIndex >= 0)
            {
                if (__bestIndex == 0)
                {
                    try
                    {
                        var typeInfo = typeInfoResolver.GetTypeInfo(typeof(global::G.VideoSegmentation0), options) as global::System.Text.Json.Serialization.Metadata.JsonTypeInfo<global::G.VideoSegmentation0> ??
                                       throw new global::System.InvalidOperationException($"Cannot get type info for {typeof(global::G.VideoSegmentation0).Name}");
                        videoSegmentation0 = global::System.Text.Json.JsonSerializer.Deserialize(__rawJson, typeInfo);
                    }
                    catch (global::System.Text.Json.JsonException)
                    {
                    }
                    catch (global::System.InvalidOperationException)
                    {
                    }
                }
                else if (__bestIndex == 1)
                {
                    try
                    {
                        var typeInfo = typeInfoResolver.GetTypeInfo(typeof(global::G.VideoSegmentation1), options) as global::System.Text.Json.Serialization.Metadata.JsonTypeInfo<global::G.VideoSegmentation1> ??
                                       throw new global::System.InvalidOperationException($"Cannot get type info for {typeof(global::G.VideoSegmentation1).Name}");
                        videoSegmentation1 = global::System.Text.Json.JsonSerializer.Deserialize(__rawJson, typeInfo);
                    }
                    catch (global::System.Text.Json.JsonException)
                    {
                    }
                    catch (global::System.InvalidOperationException)
                    {
                    }
                }
            }

            if (videoSegmentation0 == null && videoSegmentation1 == null)
            {
                try
                {

                    var typeInfo = typeInfoResolver.GetTypeInfo(typeof(global::G.VideoSegmentation0), options) as global::System.Text.Json.Serialization.Metadata.JsonTypeInfo<global::G.VideoSegmentation0> ??
                                   throw new global::System.InvalidOperationException($"Cannot get type info for {typeof(global::G.VideoSegmentation0).Name}");
                    videoSegmentation0 = global::System.Text.Json.JsonSerializer.Deserialize(__rawJson, typeInfo);
                }
                catch (global::System.Text.Json.JsonException)
                {
                }
                catch (global::System.InvalidOperationException)
                {
                }
            }

            if (videoSegmentation0 == null && videoSegmentation1 == null)
            {
                try
                {

                    var typeInfo = typeInfoResolver.GetTypeInfo(typeof(global::G.VideoSegmentation1), options) as global::System.Text.Json.Serialization.Metadata.JsonTypeInfo<global::G.VideoSegmentation1> ??
                                   throw new global::System.InvalidOperationException($"Cannot get type info for {typeof(global::G.VideoSegmentation1).Name}");
                    videoSegmentation1 = global::System.Text.Json.JsonSerializer.Deserialize(__rawJson, typeInfo);
                }
                catch (global::System.Text.Json.JsonException)
                {
                }
                catch (global::System.InvalidOperationException)
                {
                }
            }

            var __value = new global::G.VideoSegmentation(
                videoSegmentation0,

                videoSegmentation1
                );

            return __value;
        }

        /// <inheritdoc />
        public override void Write(
            global::System.Text.Json.Utf8JsonWriter writer,
            global::G.VideoSegmentation value,
            global::System.Text.Json.JsonSerializerOptions options)
        {
            options = options ?? throw new global::System.ArgumentNullException(nameof(options));
            var typeInfoResolver = options.TypeInfoResolver ?? throw new global::System.InvalidOperationException("TypeInfoResolver is not set.");

            if (value.IsVideoSegmentation0)
            {
                var typeInfo = typeInfoResolver.GetTypeInfo(typeof(global::G.VideoSegmentation0), options) as global::System.Text.Json.Serialization.Metadata.JsonTypeInfo<global::G.VideoSegmentation0?> ??
                               throw new global::System.InvalidOperationException($"Cannot get type info for {typeof(global::G.VideoSegmentation0).Name}");
                global::System.Text.Json.JsonSerializer.Serialize(writer, value.VideoSegmentation0!, typeInfo);
            }
            else if (value.IsVideoSegmentation1)
            {
                var typeInfo = typeInfoResolver.GetTypeInfo(typeof(global::G.VideoSegmentation1), options) as global::System.Text.Json.Serialization.Metadata.JsonTypeInfo<global::G.VideoSegmentation1?> ??
                               throw new global::System.InvalidOperationException($"Cannot get type info for {typeof(global::G.VideoSegmentation1).Name}");
                global::System.Text.Json.JsonSerializer.Serialize(writer, value.VideoSegmentation1!, typeInfo);
            }
        }
    }
}