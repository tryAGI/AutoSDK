# AutoSDK
The goal of this project is to automate and minimize the effort of maintaining high-quality .NET SDKs generated based on OpenAPI and AsyncAPI specifications, mainly to strengthen the AI ecosystem within .NET. The code generated by this library is actively used in [more than 200 SDK repositories of varying complexity levels](#examples-of-use-in-real-sdks) and in [the LangChain.NET project](https://github.com/tryAGI/LangChain), which ensures that it is tested in various scenarios and is ready for them.
Inspired by [NSwag](https://github.com/RicoSuter/NSwag) ❤️.

## Ecosystem maintenance

Our goal is to make AutoSDK the best possible generator for first-class .NET clients by using every useful capability that can be derived safely from OpenAPI and AsyncAPI specifications. AutoSDK currently powers more than 200 SDK repositories, so a generator-level improvement can improve or repair the entire applicable SDK fleet instead of only one client.

The tryAGI [SDK audit](https://github.com/tryAGI/tryAGI/blob/main/GENERATED_SDK_AUDITS.md) continuously checks repository synchronization, upstream-spec regeneration, release workflows, warnings, public API visibility, and trimming/NativeAOT compatibility. Every issue reported in AutoSDK or a generated SDK is first investigated for ecosystem-wide applicability. When the root cause belongs in the generator, we fix and regression-test AutoSDK, then roll the result out to every applicable SDK. Provider-specific behavior remains in the affected repository when it cannot be derived safely from the specification.
...
- Used in more than 200 real SDK repositories and adapted to solve a wide range of API shapes
...
The generated `AutoSDKPager.OffsetAsync<TPage, TItem>(...)` overload takes a fetch-page delegate, an item extractor, and an optional `has_more` predicate; `AutoSDKPager.CursorAsync<TPage, TItem>(...)` takes a fetch-by-cursor delegate plus a next-cursor extractor. `AutoSDKPager.NextUrlAsync<TPage, TItem>(...)` follows a response URL and checks that absolute URLs stay on the client's origin. All expose results through `IAsyncEnumerable<TItem>` and respect `EnumeratorCancellation`. Auto-detection covers three pagination shapes:

- **Offset style**: a `page`/`page_number`/`pageNumber`/`pageIndex` query parameter plus a response with a single array property. Generates `<Method>AutoPagingAsync(...page_number = 1)` backed by `AutoSDKPager.OffsetAsync`. When the response also exposes a boolean `has_more`/`has_next`/`has_next_page` (or `more`) property, the detector wires it into the pager's `hasMore` predicate so paging stops on an explicit "no more pages" signal rather than waiting for an empty page.
- **Cursor style**: a `cursor`/`after`/`page_token`/`next_page`/`next_page_token`/`pagination_token` query parameter plus a response with a single array property AND a string-typed next-cursor field (`next_cursor`/`next_page`/`next_page_token`/`next_page_cursor`/`after`/`last_id`). Generates `<Method>AutoPagingAsync(...cursor = null)` backed by `AutoSDKPager.CursorAsync`.
- **Next-URL style**: a response with one array property and either a string `links.next` property (JSON:API, including App Store Connect), `next_url`, or `next_link`. Generates `<Method>AutoPagingAsync(..., requestOptions = null)` backed by `AutoSDKPager.NextUrlAsync`. The generated method retains the operation's filter, include, fields, limit, and other parameters. Each next URL is sent through the ordinary generated request pipeline with authentication, hooks, timeout, and cancellation. Relative URLs are resolved against the API origin; cross-origin URLs are rejected. The URL is used without appending the original query parameters again. Repeated URLs stop enumeration.

Next-URL takes precedence over offset, which takes precedence over cursor when multiple shapes match. Detection is conservative: skips multi-array responses, ambiguous next-URL or next-cursor candidates, and any non-GET shape. For an ambiguous response, add operation extensions `x-autosdk-pageable-items: data` and `x-autosdk-pageable-next-url: links.next` (use the JSON property names, with one optional nesting level) to select the item array and URL explicitly. An unresolved explicit path suppresses generation. Disable generated companions and pager support with `--generate-pageable-helpers false` or `<AutoSDK_GeneratePageableHelpers>false</AutoSDK_GeneratePageableHelpers>`; callers can also use the raw operation and `AutoSDKPager` directly.
...
A context registers only its package's share and lets System.Text.Json discover the rest of the family implicitly, through properties. That is a problem when the name STJ derives for a discovered type collides with another type's — `Nullable<RepositorySquashMergeCommitTitle>` against a schema that really is called `NullableRepositorySquashMergeCommitTitle`. Single-project generation never hits it, because one context sees both halves and names them apart. So the family-wide colliding types are added to every context that is not `Core`'s, which is what lets each one disambiguate; `scripts/test-split-by-tags-large-spec.sh` fails on any `SYSLIB1031` so the guarantee does not quietly lapse.
