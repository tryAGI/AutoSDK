...
- **Offset style**: a `page`/`page_number`/`pageNumber`/`pageIndex` query parameter plus a response with a single array property. Generates `<Method>AutoPagingAsync(...page_number = 1)` backed by `AutoSDKPager.OffsetAsync`.
